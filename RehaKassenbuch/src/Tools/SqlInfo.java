...
import rehaKassenbuch.RehaKassenbuch;
...
/*******************************************/
	public static int holeId(String tabelle, String befehl){
		int retid = -1;
		Statement stmt = null;
		ResultSet rs = null;
...
/*************************************/
	public static String holeEinzelFeld(String xstmt){
		...
		try{
			RehaKassenbuch.thisFrame.setCursor(new Cursor(Cursor.WAIT_CURSOR));
			String sstmt = xstmt;
			rs = stmt.executeQuery(sstmt);
			while(rs.next()){
			...
			RehaKassenbuch.thisFrame.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
			return ret;
		}catch(SQLException ev){
			//System.out.println("SQLException: " + ev.getMessage());
			//System.out.println("SQLState: " + ev.getSQLState());
