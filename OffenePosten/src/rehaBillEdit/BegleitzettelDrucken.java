	...
	private void ersetzePlatzhalter(){
		...
		for (int i = 0; i < placeholders.length; i++) {
			if(placeholders[i].getDisplayText().toLowerCase().equals("<gkv1>")){
				...
				placeholders[i].getTextRange().setText(OffenePosten.hmFirmenDaten.get("Firma1")+" "+
						OffenePosten.hmFirmenDaten.get("Firma2")+"\n"+
						OffenePosten.hmFirmenDaten.get("Strasse")+"\n"+
						OffenePosten.hmFirmenDaten.get("Plz")+" "+
						OffenePosten.hmFirmenDaten.get("Ort"));
			}else if(placeholders[i].getDisplayText().toLowerCase().equals("<gkv8>")){
				placeholders[i].getTextRange().setText(OffenePosten.aktIK);
