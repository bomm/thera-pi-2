...
//public class RehaSmartDialog extends JXDialog implements RehaTPEventListener{
	...
	public RehaSmartDialog(JXFrame owner,String name) {
		...
		super(owner, (JComponent)Reha301.thisFrame.getGlassPane());
		//super(owner,null);
		this.name = name;
		this.setName(name);
		...
		try {
			...
			UIManager.setLookAndFeel(Reha301.aktLookAndFeel);
	...
	*/
	...
	public void windowActivated(java.awt.event.WindowEvent e) {    
		//Reha301.thisClass.shiftLabel.setText("Dialog -Focus da");
		if(pinPanel != null){
	...
	public void windowDeactivated(java.awt.event.WindowEvent e) {    
		//Reha301.thisClass.shiftLabel.setText("Dialog -Focus weg");
		if(pinPanel != null){
			pinPanel.SetzeAktivButton(false);
		}
	}   
	...
	public void mouseReleased(java.awt.event.MouseEvent e) {    
		...
		SwingUtilities.invokeLater(new Runnable(){
			public  void run(){
				setCursor(Reha301.thisClass.cdefault);
				clickX = -1;
				clickY = -1;
				orgbounds[0] = -1;
				orgbounds[1] = -1;					
	...
	public void mouseDragged(java.awt.event.MouseEvent e) {
		...
		if (! insize && clickY > 0){
			...
			SwingUtilities.invokeLater(new Runnable(){
				public  void run(){
					setCursor(Reha301.thisClass.cmove);
...
//				public  void run(){
			...
			for(int i = 0;i<1;i++){
				if(sizeart==1){ //nord-west
					...
					setCursor(Reha301.thisClass.cnwsize);
					break;
				}
				if(sizeart==2){ //nord-ost
					...
					setCursor(Reha301.thisClass.cnesize);
					break;
				}
				if(sizeart==3){ //nord
					dim.height = (oY > orgbounds[1] ? dim.height-(oY-orgbounds[1]) : dim.height+(orgbounds[1]-oY));						
					...
					setCursor(Reha301.thisClass.cnsize);
					break;
				}	
				if(sizeart==4){ //s�d-west
					...
					setCursor(Reha301.thisClass.cswsize);
					break;
				}
				if(sizeart==5){ //west
					dim.width = (oX > orgbounds[0] ? dim.width-(oX-orgbounds[0]) : dim.width+(orgbounds[0]-oX));
					...
					setCursor(Reha301.thisClass.cwsize);
					break;
				}
				...
				if(sizeart==7){ //s�d
					...
					setCursor(Reha301.thisClass.cssize);
					break;
				}
				if(sizeart==8){ //ost
					dim.width = (oX > orgbounds[0] ? dim.width+(oX-orgbounds[0]) : dim.width-(orgbounds[0]-oX));
					...
					setCursor(Reha301.thisClass.cesize);
					break;
				}
	...
	public void mouseMoved(java.awt.event.MouseEvent e) {    
		for(int i = 0; i < 1; i++){
			...
			if ((e.getX()>=  (((JComponent) e.getSource()).getWidth()-4)) && //s�d-ost
				...
				orgbounds[0]=e.getXOnScreen();
				orgbounds[1]=e.getYOnScreen();						
				setCursor(Reha301.thisClass.csesize);
				break;
			}
