...
public class OOTools{
		...
		while ((start = stext.indexOf("^")) >= 0){
			...
			for(int i = 1;i < 150;i++){
				if(stext.substring(start+i,start+(i+1)).equals("^")){
					...
					Object ret = JOptionPane.showInputDialog(null,"<html>Bitte Wert eingeben f�r: --\u003E<b> "+sanweisung+" </b> &nbsp; </html>","Platzhalter gefunden", 1);
					if(ret==null){
						return true;
							//sucheErsetze(dummy,"");
					}else{
	...
	public static void holeClipBoard() {
		try {
			...
			xMultiComponentFactory = (XMultiComponentFactory) Reha301.officeapplication.getDocumentService();
